...
bool send_winhttp_post(const wchar_t* server, INTERNET_PORT port, const wchar_t* path, const std::vector<char>& payload, std::vector<char>& response_out) {
    ...
    // FIXED: Added required Vanguard gateway headers (X-VG-2, X-VG-3) alongside Content-Type
    const wchar_t* headers = L"Content-Type: application/x-protobuf\r\nX-VG-2: 153fad21-41cb-5919-b7a1-4178dba5e019\r\nX-VG-3: 1\r\n";
    bool bResults = WinHttpSendRequest(hRequest, headers, (DWORD)-1L, (LPVOID)payload.data(), (DWORD)payload.size(), (DWORD)payload.size(), 0);
...
void simulate_gateway() {
    ...
    const wchar_t* SERVER_URL = L"eu.vg.ac.pvp.net"; // Gerçek Riot Vanguard sunucusu
    INTERNET_PORT PORT = 8443; // Vanguard HTTPS iletişim portu
    const wchar_t* PATH = L"/vanguard/v1/gateway"; // Ana gateway endpoint'i
    ...
    if (auth_file.is_open()) {
        ...
        if (auth_file.read(buffer.data(), size)) {
            ...
            if (send_winhttp_post(SERVER_URL, PORT, PATH, buffer, response)) {
    ...
    log_message("Gateway authentication complete. Sunucuya alindi (Admitted to server).");
}
